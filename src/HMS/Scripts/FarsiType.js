...
FarsiType.init = function() {
    ...
    for (var j = 0; j < Areas.length; j++) {
        if (Areas[j].lang.toLowerCase() == 'fa' || Areas[j].lang.toLowerCase() == 'fa-ir') {
            FarsiType.counter++;
            new FarsiType.KeyObject(Areas[j], FarsiType.counter);
        }
    }
            ...
            if (key > 128) {
                if (FarsiType.KeyBoardError == 0) {
                    ...
                    swal({
                        position: 'center',
                        type: 'success',
                        title: 'لطفا زبان سیستم خود را بر روی انگلیسی قرار دهید',
            ...
            if (FarsiType.Type && z.farsi) {
                ...
                if ((key >= 65 && key <= 90 && !e.shiftKey) || (key >= 97 && key <= 122) && e.shiftKey) {
                    swal({
                        position: 'center',
                        type: 'success',
                        title: 'لطفا کپس لاک سیستم خود را خاموش نمایید',
                        confirmButtonText: 'بسیار خوب',
                        showConfirmButton: true,
                        showCloseButton: true,
                        target: document.getElementById('rtl-container')
