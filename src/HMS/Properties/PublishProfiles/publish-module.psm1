...
function InternalNormalize-MSDeployUrl{
    ...
    process{
        ...
        if(-not [string]::IsNullOrWhiteSpace($serviceUrl)){
            if([string]::Compare($serviceMethod,'WMSVC',[StringComparihms]::OrdinalIgnoreCase) -eq 0){
                # if no http or https then add one
                if(-not ($serviceUrl.StartsWith($httpStr,[StringComparihms]::OrdinalIgnoreCase) -or 
                            $serviceUrl.StartsWith($httpsStr,[StringComparihms]::OrdinalIgnoreCase)) ){
                ...
                if( ([string]::Compare('https',$serviceUriBuilder.Scheme,[StringComparihms]::OrdinalIgnoreCase) -eq 0) -and
                     -not $serviceUrl.Contains((':{0}' -f $serviceUriBuilder.Port)) ) {
                    $serviceUriBuilder.Port = 8172
                }

                # if no path then add one
                if([string]::Compare('/',$serviceUriBuilder.Path,[StringComparihms]::OrdinalIgnoreCase) -eq 0){
            ...
            elseif([string]::Compare($serviceMethod,'RemoteAgent',[StringComparihms]::OrdinalIgnoreCase) -eq 0){
                [System.UriBuilder]$serviceUriBuilder = New-Object -TypeName 'System.UriBuilder' $serviceUrl
